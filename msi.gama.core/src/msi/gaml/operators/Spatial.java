...
public abstract class Spatial {
			...
			try {
				...
				try {
					...
					geom = geom1.buffer(0.01, BufferParameters.DEFAULT_QUADRANT_SEGMENTS, BufferParameters.CAP_FLAT)
						.intersection(
							geom2.buffer(0.01, BufferParameters.DEFAULT_QUADRANT_SEGMENTS, BufferParameters.CAP_FLAT));
				}
			}
	...
	public static abstract class Transformations {
		...
		public static IShape without_holes(final IScope scope, final IShape g) {
			...
			if ( geom instanceof Polygon ) {
				result = GeometryUtils.FACTORY.createPolygon(
					GeometryUtils.FACTORY.createLinearRing(((Polygon) geom).getExteriorRing().getCoordinates()), null);
				...
				for ( int i = 0; i < mp.getNumGeometries(); i++ ) {
					...
					polys[i] = GeometryUtils.FACTORY.createPolygon(
		...
		public static IShape simplification(final IScope scope, final IShape g1, final Double distanceTolerance) {
			...
			if ( geomSimp != null && !geomSimp.isEmpty() &&
				geomSimp.isSimple() ) { return new GamaShape(g1, geomSimp); }
	...
	public static abstract class Properties {
		...
		@doc(
			...
			examples = { @example(value = "polyline([{10,10},{20,20}]) touches {15,15}", equals = "false"),
	...
	public static abstract class Queries {
		...
		@doc(value = "An agent, the closest to the operand (casted as a geometry).",
			...
			examples = { @example(value = "agent_closest_to(self)",
				...
				test = false) },
			see = { "neighbours_at", "neighbours_of", "agents_inside", "agents_overlapping", "closest_to", "inside",
				"overlapping" })
